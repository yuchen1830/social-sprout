# AI Prompt Template for Frontend Components

Use this template to quickly generate UI components that are type-safe and aligned with our backend contracts.

## Instructions
1. Open your AI coding assistant (Cursor, Copilot, ChatGPT, etc.).
2. Copy the **Context** and the **Target Task** below.
3. Open `lib/contracts.ts` and copy its entire content.
4. Paste everything together into the AI chat.

---

## 1. Context (Copy this)

I am building the frontend for **SocialSprout**, a Next.js (App Router) application.
**Tech Stack**:
- TypeScript
- TailwindCSS (Styling)
...
I need you to build a component that strictly follows the Zod schemas I will provide. The form inputs must match the schema keys exactly.

## 2. Target Task (Example: Campaign Form)

**Task**: Build a `UnifiedCampaignForm` component.
**Goal**: Allow a user to create a campaign and trigger generation in one step.
**Input Schema**: `CreateCampaignInputSchema` (from the code below).
**Features**:
- Use `useForm` with `zodResolver`.
- Group fields logically: Brand Info, Campaign Goal, Platforms (Checkbox group), Styling (Select), Budget (Number), and Assets (Stub component called `AssetUploader`).
- Display any validation errors from Zod.
- On submit, log the JSON payload to the console (mock submission).

## 3. The Contract (Copy content of lib/contracts.ts)
